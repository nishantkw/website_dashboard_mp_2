...
const FilterSelect = memo(function FilterSelect({
  ...
  return (
    <div className="flex shrink-0 flex-col items-center">
      <label
        className="mb-1 block w-full text-center text-[9px] font-bold uppercase tracking-wider text-[#1a5c38]"
        title={column}
      >
        {label}
      </label>
...
export default function FraudFilterBar({
  ...
  onClear,
  activeCount,
  subtitle = 'FRS §5 — schema columns only (dmart_mp fraud tables)',
}: FraudFilterBarProps) {
  ...
  return (
    <div className="mb-5 rounded-xl border border-[#b8dcc4] bg-white shadow-[0_4px_20px_-4px_rgba(26,92,56,0.18)]">
      ...
      <div className="scrollbar-visible flex gap-2.5 overflow-x-auto px-3 pb-3 pt-3 sm:px-4">
        ...
        {fields.some((f) => f.key === 'trigger_date_from') && (
          <div className="flex shrink-0 flex-col items-center">
            ...
            <div className="flex items-center gap-1">
              <input
                ...
                className="w-[110px] rounded-lg border border-[#c5e0ce] bg-white px-2 py-1.5 text-xs outline-none focus:border-[#2d8a4e]"
